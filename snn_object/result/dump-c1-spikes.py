...
print('S1 layer creation gabor_input_layers took {} s'.format(time.clock() - t1))
t2 = time.clock()
nw.create_cross_layer_inhibition(layer_collection['S1'])
print('S1 layer creation layer_inhibition took {} s'.format(time.clock() - t2))
...
print('C1 creation create_local_inhibition took {} s'.format(time.clock() - t2))
...
for filename, target_img in imgs:
    ...
    print('set_i_offsets_for_all_scales_to', time.clock() - t2, 'seconds')
    t1 = time.clock()
