...
{
const int thrd_n = kernel().vp_manager.get_thread_id();
bool init_flag = nest::kernel().simulation_manager.get_init_flag(thrd_n);

if(init_flag == true){
 int num=nest::kernel().simulation_manager.get_num_neuron(thrd_n);
 nest::kernel().simulation_manager.set_V_1(thrd_n,num,V_.P20_,V_.P11ex_,V_.P11in_,V_.P21ex_,V_.P21in_, V_.P22_,V_.weighted_spikes_ex_ ,V_.weighted_spikes_in_,V_.RefractoryCounts_);
 nest::kernel().simulation_manager.set_S_1(thrd_n,num,S_.i_0_,S_.i_1_,S_.i_syn_ex_,S_.i_syn_in_,S_.V_m_,S_.r_ref_);
 nest::kernel().simulation_manager.set_P_1(thrd_n,num,P_.I_e_,P_.Theta_,P_.V_reset_);
}
...
nest::iaf_psc_exp_ps::handle( SpikeEvent& e )
{
  ...
  double thrd_n = kernel().vp_manager.get_thread_id();
  if ( e.get_weight() >= 0.0 )
  {
   ...
   kernel().simulation_manager.handle_weight_ex(thrd_n,this->get_gid(),(e.get_weight() * e.get_multiplicity()) );
  }
  else
  {
   ...
   kernel().simulation_manager.handle_weight_in(thrd_n,this->get_gid(),(e.get_weight() * e.get_multiplicity())  );
  }
}
