...
nest::SimulationManager::simulate( Time const& t )
{
  prepare();
  init_sharemem_para();
double t_temp = what_time_is_it_now();
  run( t );
//cout<<"time_update_FPGA:"<<time_update_FPGA<<endl;
cout<<"======== thrd 1========="<<endl;
cout<<"time_deliver:"<<time_deliver[0]<<endl;
cout<<"time_iaf_init:"<<iaf_init[0]<<endl;
cout<<"time_iaf_update_:"<<iaf_update_[0]<<endl;
cout<<"time_iaf_emit_spike:"<<iaf_back[0]<<endl;
cout<<"time_others:"<<time_others[0]<<endl;
cout<<"time_neuron:"<<time_neuron[0]<<endl;
cout<<"======== thrd 2========="<<endl;
cout<<"time_deliver:"<<time_deliver[1]<<endl;
cout<<"time_iaf_init:"<<iaf_init[1]<<endl;
cout<<"time_iaf_update_:"<<iaf_update_[1]<<endl;
cout<<"time_iaf_emit_spike:"<<iaf_back[1]<<endl;
cout<<"time_others:"<<time_others[1]<<endl;
cout<<"time_neuron:"<<time_neuron[1]<<endl;
cout<<"time_gather:"<<time_gather<<endl;
cout<<"========  run =========="<<endl;
cout<<"time_run:"<<what_time_is_it_now()-t_temp<<endl;
  unmap_sharemem_para();
  cleanup();
}
...
time_deliver[thrd]+=what_time_is_it_now()-t1;
#ifdef HAVE_MUSIC
...
t1= what_time_is_it_now();
// ===================================initial parameter =======================================
int offset=((int)QUERTER_INPUT_2)/4;
if(clock_.get_tics()==0){
      init_flag[thrd]=true;
      num_neuron[thrd]=0;
      num_others[thrd]=0;
      for (
        ...
        ++node )
      {
        ...
        try
        {
          if ( not( *node )->is_frozen() )
          {
           ...
           if(( *node )->get_model_id()==IAF_PSC_EXP_ID)
            {
                   ( *node )->update( clock_, from_step_, to_step_ );
                    input01[offset+2*num_neuron[thrd]]=0;
                    gid[thrd][( *node )->get_gid()]=num_neuron[thrd];
                    num_g[thrd][num_neuron[thrd]]=( *node )->get_gid();
                    num_neuron[thrd]++;
             }
           else if (( *node )->get_model_id()!=IAF_PSC_EXP_ID)
            {
                    num_otherg[thrd][num_others[thrd]]=( *node )->get_gid();
                    num_others[thrd]++;
            }
...
iaf_init[thrd]+=what_time_is_it_now()-t1;
//// ===================================init fire=============================
/*
t1= what_time_is_it_now();
if(thrd==0){
   for(int num=0;num<num_neuron[thrd];num++)
   {
   input01[offset+2*num]=0;
   }
}
if(thrd==1){
   for(int num=0;num<num_neuron[thrd];num++)
   {
   input11[offset+2*num]=0;
   }
}
iaf_back[thrd]+=what_time_is_it_now()-t1;
*/
// ===================================FPGA Neuron Update=============================
...
if(thrd==0){
  t1 = what_time_is_it_now();
  iaf_update(XIAF_PSC_EXP_PS_TOP_CTRL_BUS_ADDR_BASEADDR0,1,num_neuron[0],
            INPUT_STREAM0,INPUT_STREAM1,INPUT_STREAM2,INPUT_STREAM3);
  iaf_update_[0]+=what_time_is_it_now()-t1;
  }


// ===================================emit spike ========================================
t1 = what_time_is_it_now();
init_flag[thrd]=false;
    if(thrd==0)
    {
        const float EPSINON = 0.1;
	for(int num=0;num<num_neuron[thrd];num++){
	
	    if(input01[offset+2*num]>=EPSINON)
	    {
		Node* updateNode=kernel().node_manager.get_node(num_g[thrd][(int)input01[offset+2*num]]);
		updateNode->update(  clock_, from_step_, to_step_);
		input03[4*(int)input01[offset+2*num]+2]=V_1_RefractoryCounts_[thrd][(int)input01[offset+2*num]];
		input03[4*(int)input01[offset+2*num]+3]=P_1_V_reset_[thrd][(int)input01[offset+2*num]];
        input01[offset+2*num]=0;
	    }
            else
            {
...
// =================================== other node (spike detector and others) ==============================
t1 = what_time_is_it_now();
for(int num=0;num<num_others[thrd];num++)
{
      Node* updateNode=kernel().node_manager.get_node(num_otherg[thrd][num]);
      updateNode->update(  clock_, from_step_, to_step_);
}
time_others[thrd]+=what_time_is_it_now()-t1;
time_neuron[thrd]+=what_time_is_it_now()-t0;
...
#pragma omp master
      {
        ...
        if ( to_step_ == kernel().connection_manager.get_min_delay() )
        {
          ...
          time_gather+=what_time_is_it_now()-t1;
        }
